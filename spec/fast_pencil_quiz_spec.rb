...
describe FastPencilQuiz do
	...
	context "when #run is run" do
		before do
			@dictionary_file_name = "path to dictionary_file"
			@mock_file_dictionary = mock(File)
			@fast_pencil_quiz.should_receive(:dictionary_file_name).and_return(@dictionary_file_name)
			File.should_receive(:open).with(@dictionary_file_name, "r").and_yield(@mock_file_dictionary)
		...
		context "when #find_sequences is run" do
			context "with an empty word list" do
				before do
					@words = []
					@mock_file_dictionary.should_receive(:each).with(no_args()).and_yield(nil)
					@number_of_sequences = @fast_pencil_quiz.find_sequences(@mock_file_dictionary)
				end
			...
			context "with a valid word list" do
				before do
					...
					@mock_file_dictionary.should_receive(:each).with(no_args()).and_yield("arrows").and_yield("food").and_yield("carrots").and_yield("give").and_yield("me").and_yield("food")
					@number_of_sequences = @fast_pencil_quiz.find_sequences(@mock_file_dictionary)
				end
